...
요일·시각별 전체 스케줄 표는 현재 알려진 운전 필드보다 큰 vendor 상태 영역에 있을 수
있습니다. 이를 확인할 때는 다음과 같이 한 항목씩 통제된 A/B 조사를 수행합니다.

1. `jebao-flowctl probe <IP> --decode --json`으로 변경 전 raw 상태 저장
2. Jebao 앱에서 스케줄 한 항목만 변경
3. 같은 명령으로 변경 후 raw 상태 저장
4. 두 상태를 비교해 변경 위치와 인코딩을 확인

이 조사도 `jebao-flowd`에서는 read만 수행합니다. 앱이 보낸 클라우드 요청 자체는 현재
아키텍처에서 가로채거나 복호화하지 않습니다.
